 ...
 * The Comfy "C".
 *
 * The brand's own logomark, taken from Comfy's site assets and recoloured to
 * `currentColor` so it sits in a menu row or a dialog title at whatever colour
 * the surrounding text is. Paired with {@link ComfyWordmark}, which is the same
 * letterform's full word.
 */
export function ComfyMark({
  className = "w-4 h-4",
  label,
}: {
  className?: string;
  /** Set only where the mark stands alone; beside a text label it is decorative. */
  label?: string;
}) {
  return (
    <svg
