...
const filenameIconMap = { "*.ts": FileCodeIcon };
...
const highlight = async (code: string, language?: BundledLanguage, themes?: CodeOptionsMultipleThemes["themes"]) => {
  const { highlightCode } = await import("./code-highlight");
  return highlightCode(code, language, themes);
};
...
/** Compact, shared read-only code surface without file chrome. */
export function AppCodeSnippet({
  code,
  language = "json",
  className,
}: {
  ...
  return (
    <CodeBlock
      ...
      className={cn("h-auto border-0", className)}
    >
      <CodeBlockItem value={language} lineNumbers={false}>
