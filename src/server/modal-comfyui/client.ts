...
export function createModalClient(timeoutMs=30_000, options: Partial<z.infer<typeof modalExecutionLimitsSchema>> = {}) {
 ...
 async function requestUnobserved(path:string,init:RequestInit={},limit=2*1024*1024) {
  ...
  async function request(path:string,init:RequestInit={},limit=2*1024*1024) {
   return measureProvider("modal", () => requestUnobserved(path, init, limit));
  }
 ...
 return {
  ...
  async upload(source:string,media:"image"|"video"|"audio"="image",idempotencyKey?:string) {
   ...
   const result=z.object({name:z.string().regex(/^[a-zA-Z0-9_.-]+$/).max(160),media_type:z.literal(media)}).parse(await json("/api/uploads/"+media,{method:"POST",headers:idempotencyKey?{"Idempotency-Key":idempotencyKey}:undefined,body:form}));
