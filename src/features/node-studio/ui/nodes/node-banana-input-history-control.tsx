...
export function NodeBananaInputHistoryControl({
  ...
  return (
    <>
      ...
      >
        <FolderOpen className="h-4 w-4" aria-hidden="true" />{selectedAssetId ? tc("Replace") : tc("Assets")}
      </button> : null}
      <AppDialog open={open} onOpenChange={close}>
        ...
        >
          <AppDialogHeader className="flex-row items-center border-b border-neutral-700 px-4 py-3">
            <div>
              <AppDialogTitle className="m-0 text-sm">{tc("Assets")}</AppDialogTitle>
              <AppDialogDescription className="mt-0.5 text-xs text-neutral-500">{tc("Choose a")} {tc(mediaType[0].toUpperCase() + mediaType.slice(1))} {tc("asset")}</AppDialogDescription>
            </div>
            <AppDialogClose asChild>
              <AppDialogIconButton type="button" aria-label={tc("Close assets")} className="ml-auto border-0 text-neutral-400 hover:bg-neutral-700 hover:text-white">
                <X className="h-4 w-4" aria-hidden="true" />
              </AppDialogIconButton>
            </AppDialogClose>
          </AppDialogHeader>
          <div role="tablist" aria-label={tc("Asset source")} className="flex gap-2 border-b border-neutral-700 px-3 py-2">
            ...
            }}>{tc(value[0].toUpperCase() + value.slice(1))}</button>)}
          </div>
          <div className="border-b border-neutral-700 p-3">
            <label className="flex items-center gap-2 rounded-md border border-neutral-700 bg-neutral-950/55 px-3">
              ...
              <input autoFocus value={search} onChange={(event) => setSearch(event.target.value)} placeholder={tc("Search loaded assets...")} className="h-9 min-w-0 flex-1 bg-transparent text-xs outline-none" />
            </label>
          </div>
          <div id={panelId} role="tabpanel" aria-labelledby={`${panelId}-${category}`} tabIndex={0} aria-busy={assetsQuery.isFetching} className="grid min-h-40 flex-1 grid-cols-2 gap-3 overflow-y-auto p-3 sm:grid-cols-3">
            {assetsQuery.isError ? (
              <div className="col-span-full text-center text-xs text-neutral-400">
                <p role="alert">{tc("Could not load assets. Please try again.")}</p>
                <button
                  type="button"
            ...
            {assetsQuery.isLoading ? <p role="status" className="col-span-full py-8 text-center text-xs text-neutral-400">{tc("Loading...")}</p> : null}
            {!assetsQuery.isLoading && !assetsQuery.isError && visibleAssets.length === 0 ? <p className="col-span-full py-8 text-center text-xs text-neutral-400">{tc("No assets in this category.")}</p> : null}
          </div>
          {assetsQuery.hasNextPage ? (
            <footer className="border-t border-neutral-700 p-3 text-right">
              <button type="button" className="rounded border border-neutral-600 px-3 py-1.5 text-xs hover:bg-neutral-700" disabled={assetsQuery.isFetching} onClick={() => void assetsQuery.fetchNextPage()}>{tc("Load more")}</button>
            </footer>
          ) : null}
        </AppDialogContent>
      </AppDialog>
